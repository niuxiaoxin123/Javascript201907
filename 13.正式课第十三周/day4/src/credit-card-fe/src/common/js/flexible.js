...
;(() => {
        ...
        if (!dpr && !scale) {
            ...
            if (isIPhone) {
                ...
                if (win.screen.width < 321) { // 320以下小屏手机
                    console.log('iPhone小屏手机');
                    dpr = 1.125;
            ...
            resetDpr('zhaopinchong');
            console.log('device dpr:', dpr);
            scale = 1 / dpr;
        }
        ...
        // 在特定情况下将dpr设置为1,解决嵌套在iframe时的dpr设置失效问题
        function resetDpr(requestFrom) {
            let decodeUrl = decodeURIComponent(decodeURIComponent(location.search));
            if (!requestFrom) {
                return;
            }
            let requestFromStr = 'requestFrom=' + requestFrom;
            if (decodeUrl.indexOf(requestFromStr) > -1) {
                dpr = 1;
            }
